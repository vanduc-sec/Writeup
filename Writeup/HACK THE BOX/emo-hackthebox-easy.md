...
➜  hackthebox file emo.doc
emo.doc: Composite Document File V2 Document, Little Endian, Os: Windows, Version 10.0, Code page: 1252, Subject: Handmade Argentina Handcrafted Frozen Towels yellow Frozen virtual Guatemala array Lesotho JBOD, Template: Normal.dotm, Revision Number: 1, Name of Creating Application: Microsoft Office Word, Create Time/Date: Fri Oct 30 23:48:00 2020, Last Saved Time/Date: Tue Nov  3 02:14:00 2020, Number of Pages: 1, Number of Words: 9339, Number of Characters: 53237, Security: 8

- Mình sẽ xem tổng quát xem nó có j không: oleid emo.doc

➜  hackthebox oleid emo.doc
...
- Ơ đây chúng ta có thể phần code VBA có vấn đề vì vậy chúng ta sẽ kiểm tra xem sao: olevba

# hackthebox olevba emo.doc
olevba 0.60.2 on Python 3.13.7 - [http://decalage.info/python/oletools](http://decalage.info/python/oletools)
...
- Đầu tiên chúng ta sẽ phân tích hàm này nó đang muốn xóa cụm  "][(s)]w" đi
- Các bạn dùng lệnh strings sẽ thầy rõ có một đoạn rác đầu file
...
- Hàm này là sẽ giữ 50 ký tự đàu còn ký tự từ 51 thì lấy các kí tự ở vị trí lẻ.
- Code để giả mã
...
return data.replace(JUNK_PATTERN, "")

def extract_powershell(data):
idx = data.find(POWERSHELL_MARKER)
if idx == -1:
...
idx = s.find(ENC_MARKER)
if idx == -1:
raise ValueError("-ENCOD not found")
b64 = s[idx + len(ENC_MARKER):]
b64 = re.sub(r"[^A-Za-z0-9+/=]", "", b64)
b64 += "=" * (-len(b64) % 4)
return b64

def decode_ps(b64):
return base64.b64decode(b64).decode("utf-16le", errors="ignore")

def main():
data = read_dump(DUMP_FILE)
data = remove_junk(data)
data = extract_powershell(data)
data = deinterleave(data)
b64 = extract_base64(data)
print(decode_ps(b64))
...
- Sau khi chạy code xong chúng ta sẽ có được:

 

$FN5ggmsH = (182,187,229,146,231,177,151,149,166);
$FN5ggmsH += (186,141,228,...);
$FN5ggmsH += (185,179,190,184,229,151,139,157,164,235,177,239,171,183,236,141,128,187,235,134,128,158,177,176,139);

- Chúng ta chỉ càn xor nó với 0xDF là ra flag

```python
def giai_ma_flag():
    part1 = [182, 187, 229, 146, 231, 177, 151, 149, 166]
    part2 = [186, 141, 228, 182, 177, 171, 229, 236, 239, 239, 239, 228, 181, 182, 171, 229, 234, 239, 239, 228]
    part3 = [185, 179, 190, 184, 229, 151, 139, 157, 164, 235, 177, 239, 171, 183, 236, 141, 128, 187, 235, 134, 128, 158, 177, 176, 139]
    part4 = [183, 154, 173, 128, 175, 151, 238, 140, 183, 162, 228, 170, 173, 179, 229]
